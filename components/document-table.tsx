...
export function DocumentTable({ initialDocuments, people }: { initialDocuments: DocumentSummary[]; people: PersonSummary[] }) {
  ...
  return (
    <section className="records-panel" aria-labelledby="documents-heading">
      ...
      <div className="document-table" role="table" aria-label="Indexed documents"><div className="document-table__head" role="row"><span>Document</span><span>Owner</span><span>Status</span><span>Source</span></div>{visibleDocuments.map((document) => <div className="document-table__row" key={document.id} role="row"><div><strong>{document.title}</strong><span>{document.category}{document.expiresOn ? ` · expires ${document.expiresOn}` : ''}</span></div><span>{document.personName}</span><StatusPill tone={document.status === 'review' ? 'review' : undefined}>{document.status === 'review' ? 'Needs review' : 'Indexed'}</StatusPill><button className="table-link" onClick={() => setSourceId(document.id)} type="button">View citations</button></div>)}{!visibleDocuments.length && <p className="empty-copy">No documents match these filters.</p>}</div>
      {showUpload && <UploadDrawer onClose={() => setShowUpload(false)} onUploaded={uploaded} people={people} />}
      {sourceId && <SourcePreview documentId={sourceId} onClose={() => setSourceId(null)} />}
    </section>
  );
}
