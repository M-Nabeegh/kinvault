...
describe('citationFor', () => {
  it('preserves the document and extracted field provenance exactly', () => {
    expect(
      citationFor(
        { page: 2, label: 'Expiry date', value: '2026-11-09', confidence: 0.92 },
        document,
      ),
    ).toEqual({
  ...
  it('rejects a field without a source page', () => {
    expect(() => citationFor({ label: 'Expiry date', value: '2026-11-09', confidence: 0.92 }, document)).toThrow();
  });

  it('rejects a field without a source label', () => {
    expect(() => citationFor({ page: 2, value: '2026-11-09', confidence: 0.92 }, document)).toThrow();
  });
});
